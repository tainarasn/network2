...
class PAM5Server:
    ...
    def pam5_4d_to_binary(self, pam5_signal):
        ...
        # Verificar se o número de símbolos é múltiplo de 4
        if len(symbols) % 4 != 0:
            # Truncar ou preencher o sinal para garantir que seja múltiplo de 4
            symbols = symbols[:len(symbols) - (len(symbols) % 4)]
            print(f"Símbolos truncados para múltiplo de 4: {symbols}")
