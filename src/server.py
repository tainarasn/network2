...
class PAM5Server:
    ...
    def pam5_4d_to_binary(self, pam5_signal):
        ...
        try:
            symbols = [float(s) for s in pam5_signal.split(',') if s.strip()]
        ...
        # Verificar se o número de símbolos está correto
        if len(symbols) != 2048:
            raise ValueError(f"Número incorreto de símbolos 4D-PAM5: {len(symbols)} (esperado: 2048)")
